...
Figure below gives a schematic illustration of model 1. Single headed arrows or ‘paths’ are used to define causal relationships in the model, with the variable at the tail of the arrow causing the variable at the point (or residual if there is no tail variable). Double headed arrows represent covariances or correlations.
